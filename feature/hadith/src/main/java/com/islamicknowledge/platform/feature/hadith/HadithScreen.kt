...
@Composable fun HadithScreen(modifier:Modifier=Modifier){
var query by remember{mutableStateOf("")}
val list=if(query.isBlank()) hadiths else hadiths.filter{it.bengali.contains(query,true)||it.book.contains(query,true)}
Column(modifier){
SectionHeader(title="হাদিস",modifier=Modifier.padding(16.dp))
OutlinedTextField(query,{query=it},label={Text("হাদিস খুঁজুন")},modifier=Modifier.fillMaxWidth().padding(horizontal=16.dp))
LazyColumn(contentPadding=PaddingValues(16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
items(list,key={it.id}){h->Card(Modifier.fillMaxWidth()){Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){Text(h.book+" • "+h.number,style=MaterialTheme.typography.titleMedium);Text(h.arabic,style=MaterialTheme.typography.titleLarge);Text(h.bengali,style=MaterialTheme.typography.bodyLarge);Text(h.reference,style=MaterialTheme.typography.labelMedium)}}}
}}}}