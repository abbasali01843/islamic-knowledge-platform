...
@Composable
fun LearnSalahScreen(modifier: Modifier = Modifier) {
    var filter by rememberSaveable { mutableStateOf("সব") }
    val filters = listOf("সব", "অজু", "গোসল", "কিয়াম", "রুকু", "সিজদাহ", "শেষ বৈঠক")
    val list = if (filter == "সব") lessons else lessons.filter { it.type == filter }

    Column(modifier.fillMaxSize()) {
        SectionHeader(title = "নামাজ শিক্ষা", modifier = Modifier.padding(16.dp))
        LazyRow(
            ...
            items(list) { l ->
                Card(Modifier.fillMaxWidth()) {
                    Column(
                        ...
                        if (l.arabic.isNotBlank()) {
                            Text(l.arabic, style = MaterialTheme.typography.titleLarge)
                        }
                        Text(l.body)
                    }
                }
