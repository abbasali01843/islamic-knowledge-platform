...
export const WebModulesScreen:React.FC<{onBack:()=>void}>=({onBack})=>{
 ...
 const [heading,setHeading]=useState(0); const [geo,setGeo]=useState<string>('অবস্থান নেওয়া হয়নি');
 const select=(id:ModuleId)=>{setActive(id);setQ(0);setScore(0);setAnswered(false);};
 const locate=()=>navigator.geolocation?.getCurrentPosition(p=>setGeo(p.coords.latitude.toFixed(5)+', '+p.coords.longitude.toFixed(5)),()=>setGeo('অবস্থান অনুমতি পাওয়া যায়নি'));
 ...
 return <div className="max-w-4xl mx-auto px-4 py-6 space-y-5">
  ...
  <div className="rounded-3xl p-5 bg-white dark:bg-[#1A221C] border border-[#E8EFEA] dark:border-[#3A4D43]/60">
   ...
   {active==='QIBLA'&&<div className="text-center py-8 space-y-5"><Compass className="w-24 h-24 mx-auto text-[#176B4D]" style={{transform:'rotate('+heading+'deg)'}}/><h2 className="text-xl font-black">কিবলা কম্পাস</h2><p className="text-xs text-[#717A74]">কাবার দিক পেতে GPS ও device orientation অনুমতি দিন।</p><button onClick={locate} className="px-4 py-2 rounded-xl bg-[#176B4D] text-white text-xs font-bold">GPS অবস্থান</button><p className="text-xs">{geo}</p><button onClick={()=>setHeading(h=>h+45)} className="px-4 py-2 rounded-xl border text-xs font-bold">কম্পাস পরীক্ষা</button></div>}
