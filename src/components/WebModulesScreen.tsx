...
export const WebModulesScreen:React.FC<{onBack:()=>void}>=({onBack})=>{
 ...
 return <div className="max-w-4xl mx-auto px-4 py-6 space-y-5">
  ...
  <div className="rounded-3xl p-5 bg-white dark:bg-[#1A221C] border border-[#E8EFEA] dark:border-[#3A4D43]/60">
   ...
   {active==='QIBLA'&&<div className="text-center py-8 space-y-5"><Compass className="w-24 h-24 mx-auto text-[#176B4D]" style={{transform:'rotate('+((qibla??0)-heading)+'deg)'}}/><h2 className="text-xl font-black">কিবলা কম্পাস</h2><p className="text-xs text-[#717A74]">কাবার দিক পেতে GPS ও device orientation অনুমতি দিন।</p><button onClick={locate} className="px-4 py-2 rounded-xl bg-[#176B4D] text-white text-xs font-bold">GPS অবস্থান</button><p className="text-xs">{geo}</p>{geoError&&<p className="text-xs font-semibold text-amber-700 dark:text-amber-300">{geoError}</p>}<button onClick={startCompass} className="px-4 py-2 rounded-xl border text-xs font-bold">{compassActive?'কম্পাস চালু':'কম্পাস চালু করুন'}</button>{compassError&&<p className="text-xs font-semibold text-amber-700 dark:text-amber-300">{compassError}</p>}{qibla!=null&&<p className="text-xs font-bold">কিবলা বিয়ারিং: {qibla.toFixed(1)}°</p>}</div>}
